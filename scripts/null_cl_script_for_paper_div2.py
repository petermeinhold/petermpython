...
for m1a,m1b,m2a,m2b in zip (m30ss1rh1,m30ss1rh2,m30ss2rh1,m30ss2rh2):
    ...
    m2a.mask=m1a.mask
    m2b.mask=m1a.mask
    m30ssrhdiff.append((m1a+m2a-m1b-m2b)/4.)
    frac30ssrh=(1-float(m1a.mask.sum())/len(m1a.mask))     

cl30ssrhdiff=np.array(hp.anafast(m30ssrhdiff))*1e12/frac30ss
cl30rhdiff=np.array(hp.anafast(m30rhdiff))*1e12/frac30rh
cl30ssdiff=np.array(hp.anafast(m30ssdiff))*1e12/frac30ss
cl30rhx=np.array(hp.anafast(m30rh1,m30rh2))*1e12/frac30rh  
...
for m1a,m1b,m2a,m2b in zip (m44ss1rh1,m44ss1rh2,m44ss2rh1,m44ss2rh2):
    ...
    m2a.mask=m1a.mask
    m2b.mask=m1a.mask
    m44ssrhdiff.append((m1a+m2a-m1b-m2b)/4.)
    frac44ssrh=(1-float(m1a.mask.sum())/len(m1a.mask))     

cl44ssrhdiff=np.array(hp.anafast(m44ssrhdiff))*1e12/frac44ss
cl44rhdiff=np.array(hp.anafast(m44rhdiff))*1e12/frac44rh
cl44ssdiff=np.array(hp.anafast(m44ssdiff))*1e12/frac44ss
cl44rhx=np.array(hp.anafast(m44rh1,m44rh2))*1e12/frac44rh
...
for m1a,m1b,m2a,m2b in zip (m70ss1rh1,m70ss1rh2,m70ss2rh1,m70ss2rh2):
    ...
    m2a.mask=m1a.mask
    m2b.mask=m1a.mask
    m70ssrhdiff.append((m1a+m2a-m1b-m2b)/4.)
    frac70ssrh=(1-float(m1a.mask.sum())/len(m1a.mask))     

cl70ssrhdiff=np.array(hp.anafast(m70ssrhdiff))*1e12/frac44ss
cl70rhdiff=np.array(hp.anafast(m70rhdiff))*1e12/frac44rh
cl70ssdiff=np.array(hp.anafast(m70ssdiff))*1e12/frac44ss
cl70rhx=np.array(hp.anafast(m70rh1,m70rh2))*1e12/frac44rh


fcl=open('/global/homes/p/peterm/plots/clplots/cl_nulls.pkl','wb')
...
plt.ylim([0,10000])
show()
plt.savefig('/global/homes/p/peterm/plots/clplots/cl_null_30_div2.png')
...
plt.ylim([0,10000])
show()
plt.savefig('/global/homes/p/peterm/plots/clplots/cl_null_44_div2.png')
...
plt.ylim([0,10000])
show()
plt.savefig('/global/homes/p/peterm/plots/clplots/cl_null_70_div2.png')
