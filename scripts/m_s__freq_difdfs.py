...
f70delta=glob(basefulldelta+'*7011*full*.fits')
...
for s in ['1','2','3','4','5']:
    ...
    for c in ['21','23']:
        ...
        hp.mollview(diff,title='SS'+s+' Delta_DX9 LFI'+c+'M-freq  smooth 10 deg')
        ...
        hp.mollview(diff,title='SS'+s+' Delta_DX9 LFI'+c+'S-freq  smooth 10 deg')
        ...
        hp.mollview(diff,title='SS'+s+' DX9 LFI'+c+'M-freq  smooth 10 deg')
        plt.savefig('plots/dx9_msdiffs/dx9_lfi'+c+'m-freq_ss'+s+'.png')
        diffs=pu.get_surv_diff_fname(fs[0],f70[0],freq=70,fwhm=fwhmdeg)
        hp.mollview(diff,title='SS'+s+' DX9 LFI'+c+'S-freq  smooth 10 deg')
